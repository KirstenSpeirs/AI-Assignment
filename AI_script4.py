...
def hill_climbing():
    ...
    for i in range(1000):                  # loop start
        ...
        # print("s2: ", s2)
        r1 = random.randint(0, test_size)   # random number is chose within test size
        # print("r1: ", r1)
        r2 = random.randint(0, test_size)   # a second random number is chose within test size
        # print("r2: ", r2)
...
####______Multi-Start Hill Climbing______#######
def multi_start(num):
    results = []                                # results array created
    for i in range(num):                        # loops for desired number
        results.append(hill_climbing())         # add result to array to be plotted later
    # print(results)                              # print results

    for i in range(num):
        dist = evaluate(results[i])             # get best distances to output
        plt = plot_colours(test_colours, results[i])  # The best distance is plotted
        print("                                         Best solution: ", results[i])
        print("                                         Best distance: ", dist)
        plt.savefig('fig-final' + str(i))       # saving the plotted image - optional
        plt.draw()
    plt.show()
